...
public:
    PhoenixSController(gzFile output_trace, ofstream &output_log, int twsSize, bool useTrueLRU) : Controller(output_trace, output_log), tag_working_set(), allocated(), twsSize(twsSize), useTrueLRU(useTrueLRU), dramUsage(), allocCount(0), freeCount(0), mightAlloc(0) {}
    ...
    void setup(Decoder &decoder) override {
        ...
        for (uint64_t tag = 0; tag < TAG_TABLE_SIZE*8; tag += TAG_CACHE_LINE_SIZE*8) {
            ...
            if (((tag/512)+1) % 64 == 0) {                             // doing superroot bit   (every 8 bytes of root tags)
                superroot_byte <<= 1;
                clear = eightBytesClear(root_line, c%8);
                //if (!clear) cout << "NOT CLEAR " << tag/512 << endl;
                superroot_byte |= clear ? 0 : 1;
                if (!clear) allocated.insert(c);
    ...
    void reportStats() override {
        ...
        cout << "REPORT: MIGHT COUNT: " << mightAlloc << endl;
    }
};