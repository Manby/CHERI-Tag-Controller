        ...
        for (uint64_t rl = 0; rl < ROOT_TABLE_SIZE; rl += TAG_CACHE_LINE_SIZE) {        // do one root cacheline line at a time
            for (int rb = 0; rb < TAG_CACHE_LINE_SIZE; ++rb) {                          // each root cacheline contains TAG_CACHE_LINE_SIZE bytes
                ...
                for (int rt = 0; rt < 8; ++rt) {                                        // each byte contains 8 tags
                    ...
                    cache.set(TAG_CACHE_LINE_SIZE*(8*rl+8*rb+rt) + ROOT_TABLE_SIZE + SUPERROOT_TABLE_SIZE, leaf_line);     // insert the leaf line into the cache
            ...
            cache.set(rl + SUPERROOT_TABLE_SIZE, root_line);     // insert the root line into the cache
        ...
        switch (ax.type) {
            ...
            case ACCESS_TYPE_WRITE:
                if (ax.tags == 0) {
                    ...
                    cache.doWrite(superroot_base_addr, superroot_line);
