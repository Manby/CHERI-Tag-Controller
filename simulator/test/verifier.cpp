...
int verify(BaselineController &baselineController, Controller &controller) {
    memAccess ax;
    uint16_t baselineTags, tags;
    int discrepancies = 0;

    for (uint64_t addr = 0; addr < MEMORY_SIZE; addr += 16) {
        if ((addr << 4) % (MEMORY_SIZE) == 0) cout << "VERIFY: " << ((addr << 4) / (MEMORY_SIZE)) << "/16 complete" << endl;
...
int main(int argc, char *argv[]) {
    ...
    if (!strcmp(argv[1], "morello-t")) {
        ...
        verify(baselineController, controller);
