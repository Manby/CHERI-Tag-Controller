...
In DRAM, the root table lives above the leaf table. The leaf table lives above the value portion of DRAM (see figure \ref{fig:ett}).
...
[FIG TSS!!!]
three-stage simulation pipeline; with additional visualisation stuff too
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.65\textwidth]{bbx}
	\caption{A black-box diagram explaining the inputs and outputs of the pieces of the tag controller evaluation framework.}
	\label{fig:bbx}
\end{figure}
...
\begin{itemize}
	...
	\item The visualisation tool must be able to read in both types of captured state and produce an image that accurately depicts it all.
\end{itemize}
