...
Architectures that employ a tag table will statically allocate a region of DRAM at boot-time to act as the tag cache. This of course results in a static memory cost, which is typically acceptable for client applications, but unacceptable for server/cloud applications, according to feedback from industry [SOURCE?]. Different approaches of caching tags have been explored [any more other than hierarchical; SOURCE?], which aim to reduce the DRAM tag table access frequency, but they do not address the constant DRAM space consumption overhead of the tag table itself. \textit{Efficient Tagged Memory} details a hierarchical lookup scheme that compresses tags within the tag cache, thus storing more information about the tag table within the tag cache, further increasing tag cache hit rate, but it in fact has the effect of increasing the constant DRAM space consumption slightly due to the addition of a first-layer.\\
...
As an \textbf{extension} objective, I aim to implement my algorithm in hardware, using a hardware description language, test its correctness either by designing a suite of test cases or by formal verification, and evaluate its performance on different workloads.

I would use Bluespec for this due to it being designed for hardware implementations just like this and having a suite of tools, including simulators and FPGA synthesis tools, that I can leverage.

\subsection*{FPGA Synthesis (Extension 3)}
As an \textbf{extension} objective, I aim to synthesise my implementation on an FPGA, verify its correctness, and evaluate its performance on different workloads.
...
\begin{itemize}
	...
	\item for a simulation, it can provide at least one other detail of the tag memory accesses that would be useful information to a professional analyst
\end{itemize}
...
Several memory access traces which I intend to use exist on the Computer Lab's internal Taramax network. To use them I will need access to the parent directory in which they reside.
...
\end{itemize}
Should this device fail, I will be in need of a new laptop, so I will purchase en at-least equivalently capable machine. I will be using a private GitHub remote repository to provide version control and backups of all my programs, files and documents.\\
